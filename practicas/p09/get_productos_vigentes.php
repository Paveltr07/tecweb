...
<body>
    ...
    // Obtener el parámetro 'tope' de la URI (si se proporciona)
    $tope = isset($_GET['tope']) ? intval($_GET['tope']) : 0;

    // Consultar los productos que no estén eliminados y que cumplan con la condición de unidades <= tope
    $query = "SELECT * FROM productos WHERE eliminado = 0";
    if ($tope > 0) {
        $query .= " AND unidades <= $tope";  // Solo agregar la condición si 'tope' es mayor a 0
    }
    ...
    if ($result = $link->query($query)) {
        if ($result->num_rows > 0) {
            ...
            echo '<p>No se encontraron productos vigentes con las unidades especificadas.</p>';
        }
        $result->free();
    } else {
