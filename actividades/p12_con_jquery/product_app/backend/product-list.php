<?php
include_once __DIR__.'/database.php';

// Verificar si se ha enviado el nombre del producto para validación
if (isset($_POST['nombre'])) {
    $nombre = $_POST['nombre'];

    // Consulta para verificar si el nombre ya existe
    $stmt = $conexion->prepare("SELECT COUNT(*) as count FROM productos WHERE nombre = ? AND eliminado = 0");
    $stmt->bind_param("s", $nombre);
    $stmt->execute();
    $result = $stmt->get_result();
    $data = $result->fetch_assoc();

    // Devolver resultado como JSON
    echo json_encode(['exists' => $data['count'] > 0]);
    exit; // Terminar el script aquí
}

// SE CREA EL ARREGLO QUE SE VA A DEVOLVER EN FORMA DE JSON
$data = array();

// SE REALIZA LA QUERY DE BÚSQUEDA Y AL MISMO TIEMPO SE VALIDA SI HUBO RESULTADOS
if ($result = $conexion->query("SELECT * FROM productos WHERE eliminado = 0")) {
    // SE OBTIENEN LOS RESULTADOS
    $rows = $result->fetch_all(MYSQLI_ASSOC);

    if (!is_null($rows)) {
        // SE CODIFICAN A UTF-8 LOS DATOS Y SE MAPEAN AL ARREGLO DE RESPUESTA
        foreach ($rows as $num => $row) {
            foreach ($row as $key => $value) {
                $data[$num][$key] = utf8_encode($value);
            }
        }
    }
    $result->free();
} else {
    die('Query Error: ' . mysqli_error($conexion));
}
$conexion->close();

// SE HACE LA CONVERSIÓN DE ARRAY A JSON
echo json_encode($data, JSON_PRETTY_PRINT);
?>